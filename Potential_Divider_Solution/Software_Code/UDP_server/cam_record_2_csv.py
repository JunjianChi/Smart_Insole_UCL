        ...
        if results.pose_landmarks:
            ...
            for keypoint_name, landmark_index in keypoints.items():
                ...
                if 0 <= x < color_image.shape[1] and 0 <= y < color_image.shape[0]:
                    ...
                    if point_3d:
                        row.extend(point_3d)  # 添加 x, y, z 值
                    else:
                        row.extend([None, None, None])  # 无效值
                else:
                    row.extend([None, None, None])  # 无效值
                ...
                # 显示3D坐标标签
                label = f"{keypoint_name} ({point_3d[0]:.2f}, {point_3d[1]:.2f}, {point_3d[2]:.2f})" if point_3d else f"{keypoint_name} (None, None, None)"
